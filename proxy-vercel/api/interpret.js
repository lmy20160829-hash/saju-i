...
// (점신의 "점수 + 한줄평 → 상세" 위계와 행운 아이템 카드 벤치마킹 —
//  화면의 js/interpret-render.js가 이 세 줄을 요약 카드로 그린다)
...
function buildPrompt(payload) {
  ...
  if (topicKey === 'daeun' || topicKey === 'daeunOne') {
    ...
    for (const [i, item] of list.entries()) {
      ...
      const [ds, db] = [...item.ganzhi];
      if (!STEMS[ds] || !BRANCHES[db]) return null; // 표에 없는 글자는 거부
      ...
      const [dsKo, dsEl] = STEMS[ds];
      const [dbKo, dbEl, dbMain] = BRANCHES[db];
      const isNow = kstYear >= item.startYear && kstYear < item.startYear + 10;
      daeunLines.push(
        `  · ${item.startAge}세~ (${item.startYear}년~): ${item.ganzhi} (${dsKo}${dbKo}) — ` +
        `천간 ${dsKo}(${dsEl})[나에게 ${tenGod(dayStem, ds)}], ` +
        `지지 ${dbKo}(${dbEl})[나에게 ${tenGod(dayStem, dbMain)}]` +
        (isNow ? ' ← 지금 걷는 대운' : '') +
        (i === picked ? ' ← 내가 고른 대운' : '')
      );
    }
  ...
  if (topicKey === 'year') {
    ...
    const [ysKo, ysEl, ysYang] = STEMS[yStemHanja];
    const [ybKo, ybEl, ybMain] = BRANCHES[yBranchHanja];
    const now = new Date();
    const kstYear = new Date(
      now.getTime() + (9 * 60 + now.getTimezoneOffset()) * 60 * 1000
    ).getFullYear();
    const rel = y < kstYear ? '이미 지난 해 — 돌아보기'
      ...
      : '다가올 해 — 미리 준비';
    yearLines.push(
      `- 선택한 해의 세운: ${y}년 ${yStemHanja}${yBranchHanja} (${ysKo}${ybKo}) — ` +
      `천간 ${ysKo}(${ysEl}, ${ysYang ? '양' : '음'})[나에게 ${tenGod(dayStem, yStemHanja)}], ` +
      `지지 ${ybKo}(${ybEl})[나에게 ${tenGod(dayStem, ybMain)}] (${rel})`
    );
    ...
    if (Array.isArray(list)) {
      ...
      if (host) {
        ...
        const [hsKo, hsEl] = STEMS[hs];
        const [hbKo, hbEl, hbMain] = BRANCHES[hb];
        yearLines.push(
          `- 그 해가 속한 대운: ${host.ganzhi} (${hsKo}${hbKo}) — ` +
          `${host.startAge}세~ (${host.startYear}~${host.startYear + 9}년), ` +
          `천간 ${hsKo}(${hsEl})[나에게 ${tenGod(dayStem, hs)}], ` +
          `지지 ${hbKo}(${hbEl})[나에게 ${tenGod(dayStem, hbMain)}]`
        );
